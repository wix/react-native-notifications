...
describe('Commands', () => {
    ...
    describe('getInitialAction', () => {
        it('sends to native', () => {
            ...
            (0, ts_mockito_1.verify)(mockedNativeCommandsSender.getInitialAction()).called();
        });
        it('Android - should return undefined initial action', async () => {
            react_native_1.Platform.OS = 'android';
            (0, ts_mockito_1.when)(mockedNativeCommandsSender.getInitialAction()).thenResolve();
        ...
        it('iOS - returns a promise with the initial action', async () => {
            react_native_1.Platform.OS = 'ios';
            const expectedNotification = new Notification_1.Notification({
                identifier: 'id',
                payload: {
                    action: {
                        identifier: 'id',
            ...
            (0, ts_mockito_1.when)(mockedNativeCommandsSender.getInitialAction()).thenResolve({ identifier: 'id', notification: expectedNotification, action: { identifier: 'id', text: 'text' } });
            const actual = await uut.getInitialAction();
